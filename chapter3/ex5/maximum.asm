...
global _start
_start:
     mov edi, [data_len] ; set counter
     xor ecx, ecx ; move 0 
     mov eax, [data_items + ecx * 4] ; load the first byte of data
     mov ebx, eax ; since this is the first item, eax is
                  ; the biggest

start_loop: ; start loop
     cmp ecx, edi ; check to see if we've hit the end
     je loop_exit
     inc ecx ; load next value
     mov eax, [data_items + ecx * 4]
